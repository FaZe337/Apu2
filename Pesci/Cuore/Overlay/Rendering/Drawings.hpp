...
public:
    ...
    static void TeamMiniMap(int x, int y, int radius, int teamID, float targetyaw)
    {
        ...
        ImGui::GetWindowDrawList()->AddCircle(center, radius, colOutline, 12, Modules::Radar::minimapradardotsize2);
    ...
    //--------------------------------------------------------------------------------------------------===============================================    
    static void DrawRectangleFilled(int x, int y, int w, int h, RGBA color)
    {
    ...
    static void DrawDynamicRainbowCircle(ImDrawList* drawList, const Vector2D& pos, float radius, int num_segments = 64) {
        const float segment_radians = 2.0f * M_PI / num_segments;

        for (int i = 0; i < num_segments; ++i) {
            const float a = segment_radians * i;
            const float x1 = pos.x + radius * cosf(a);
            const float y1 = pos.y + radius * sinf(a);

            const float b = segment_radians * (i + 1);
            const float x2 = pos.x + radius * cosf(b);
            const float y2 = pos.y + radius * sinf(b);

            // Calculate rainbow color based on time
            float hue = static_cast<float>(fmod(ImGui::GetTime() * 50.0, 360.0)); // Adjust the multiplier for speed
            ImU32 color = HSVToRGB(hue, 1.0f, 1.0f);

            drawList->AddLine(ImVec2(x1, y1), ImVec2(x2, y2), color);
        }
    }
    ...
    static void DrawSeer(ImDrawList* Canvas, float x, float y, int shield, int max_shield, int health) {
        ...
        Drawings::DrawHexagonFilled(Canvas, bg1, bg2, bg3, bg4, bg5, bg6, ImColor(0, 0, 0, 120));
        ...
        Drawings::DrawQuadFilled(Canvas, h1, h2, h3m, h4m, ImColor(10,10,30,60));
        Drawings::DrawQuadFilled(Canvas, h1, h2, h3, h4, ImColor(255, 255, 255, 255));
        ...
        if (max_shield == 50) {
            if (shield <= 25) {
                if (shield < 25) {
                    Drawings::DrawQuadFilled(Canvas, s1, s2, s3m, s4m, shieldCracked);
                    Drawings::DrawQuadFilled(Canvas, ss1, ss2, ss3m, ss4m, shieldCracked);
                }
                if(shield != 0)
                    Drawings::DrawQuadFilled(Canvas, s1, s2, s3, s4, shieldCol);
                ...
                if (shield != 0)
                    Drawings::DrawQuadFilled(Canvas, ss1, ss2, ss3, ss4, shieldCol);
            }
        }
            ...
            if (shield <= 25) {
                if (shield < 25) {
                    Drawings::DrawQuadFilled(Canvas, s1, s2, s3m, s4m, shieldCracked);
                    Drawings::DrawQuadFilled(Canvas, ss1, ss2, ss3m, ss4m, shieldCracked);
                    Drawings::DrawQuadFilled(Canvas, sss1, sss2, sss3m, sss4m, shieldCracked);
                }
                if (shield != 0)
                    Drawings::DrawQuadFilled(Canvas, s1, s2, s3, s4, shieldCol);
                ...
                if (shield != 0)
                    Drawings::DrawQuadFilled(Canvas, sss1, sss2, sss3, sss4, shieldCol);
            }
        }
            ...
            if (shield <= 25) {
                if (shield < 25) {
                    ...
                    Drawings::DrawQuadFilled(Canvas, ss1, ss2, ss3m, ss4m, shieldCracked);
                    Drawings::DrawQuadFilled(Canvas, sss1, sss2, sss3m, sss4m, shieldCracked);
                    Drawings::DrawQuadFilled(Canvas, ssss1, ssss2, ssss3m, ssss4m, shieldCracked);
                }
                if (shield != 0)
                    Drawings::DrawQuadFilled(Canvas, s1, s2, s3, s4, shieldCol);
                ...
                if (shield != 0)
                    Drawings::DrawQuadFilled(Canvas, ssss1, ssss2, ssss3, ssss4, shieldCol);
            }
        }
            ...
            if (shield <= 25) {
                if (shield < 25) {
                    ...
                    Drawings::DrawQuadFilled(Canvas, sss1, sss2, sss3m, sss4m, shieldCracked);
                    Drawings::DrawQuadFilled(Canvas, ssss1, ssss2, ssss3m, ssss4m, shieldCracked);
                    Drawings::DrawQuadFilled(Canvas, sssss1, sssss2, sssss3m, sssss4m, shieldCracked);
                }
                if (shield != 0)
                    Drawings::DrawQuadFilled(Canvas, s1, s2, s3, s4, shieldCol);
                ...
                if (shield != 0)
                    Drawings::DrawQuadFilled(Canvas, sssss1, sssss2, sssss3, sssss4, shieldCol);
            }
        }
    }
